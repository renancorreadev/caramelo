...
describe('token-contract', () => {
  ...
  const fetchBalance = async (account: any) => {
    return (await program.provider.connection.getParsedAccountInfo(account))
  ...
  it('Mint a token', async () => {
    ...
    const res = await anchor.AnchorProvider.env().sendAndConfirm(mintTx, [
      mintKey,
    ]);
